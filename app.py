...
db = client['ports']  # 포트번호별 컬렉션 사용 (예: "80","443"...)
...
def save(country: str, port: int, logs: list):
    ...
    results = api.search(query)
###
    # current_ips = [match["ip_str"] for match in results["matches"][:100]]
    # collection = db[f'{80}']
    # cnt = 0
    # for ip in current_ips:
    #     ip_no = collection.find_one({"IP":f"ip"})
    #     if not ip_no:
    #         cnt += 1
...
def start():
    if request.method == 'GET':
        ...
        return render_template('start.html', logs=logs, preview=None)

@app.route('/preview', methods=['GET'])
def preview():
    report = build_report_text()
    return render_template('preview.html', logs=None, preview=report)
...
def send_now():
    if request.method == 'GET':
        ...
        return render_template('send.html', logs=logs, preview=report)
